// TUTORIALREGISTER — standalone tutorial-video producer for the SparkMyName Live Agent guide.
...
// gets a BRAND-NEW name (producer + storage folder + manifest): tutorialregister is virgin, permanent, never reused.
// Old reels are frozen history — never read, never written.
// ============================================================
// Register Your Business — a single warm narrator (no customer cameos) + Lyria music + 4 Veo scenes,
// a calm instructional "watch how" tutorial cut for one Live Agent step.
// 100% isolated: its OWN file, its OWN storage folder (tutorialregister/), its OWN manifest, its OWN single film.
...
const FILM = {
  slug: "tutorialregister",   // virgin, permanent, never reused
  nm: "Register Your Business",
  ...
  script: "Let's make your business official. Search your state's website for 'register a business.' Follow their steps to set it up. This makes your business real and legal. One form, and you're on the map. You've got this."
};
...
const STORY = [
 // T1
 "A person opens a laptop at a kitchen table in morning light, ready. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T2
 "Hands move through a plain form on a laptop screen; calm, deliberate taps; text-free. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T3
 "An official seal emblem draws itself in gold, then a soft stamp motion — official, shapes only. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T4
 "The person sits back, quietly proud, a hand on the table. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts."
];
...
async function tts(){
  ...
  const up = await storage.uploadPng('tutorialregister/tutorialregister-voice.mp3', b64, 'audio/mpeg');
...
async function lyria(){
  ...
  const up = await storage.uploadPng('tutorialregister/tutorialregister-music.mp3', aud.data, mime);
...
async function veo(prompt, ix){
  ...
  for(const model of MODELS){
    for(let a=1;a<=2;a++){
      ...
      if(!r.ok){
        ...
        console.error('TUTORIALREGISTER veo start FAIL',ix,model,'try',a,r.status,t.slice(0,240));
        ...
        if(/personGeneration|person_generation/i.test(t)){
          ...
          if(r.ok){ op=await r.json(); console.log('TUTORIALREGISTER veo OK (no-person)',model,ix); break outer; }
          lastErr='veo '+model+' person retry '+r.status; break; }
        lastErr='veo '+model+' '+r.status+' '+t; break;
      } else { op=await r.json(); console.log('TUTORIALREGISTER veo OK',model,ix); break outer; }
    }
  }
  ...
  const up = await storage.uploadPng('tutorialregister/tutorialregister-scene'+ix+'.mp4', b64, 'video/mp4');
...
async function saveManifest(man){
  try{
    ...
    await storage.uploadPng('tutorialregister/manifest.json', mb64, 'application/json');
  }catch(e){ console.error('TUTORIALREGISTER MANIFEST SAVE FAIL', e.message); }
}
exports.handler = async (event) => {
  ...
  console.log('TUTORIALREGISTER BUILD RUN',RUN,'redo='+(q.redo||'(none)')+' only='+(q.only||'(none)'));
  console.log('TUTORIALREGISTER KEYS openai:'+(OA?'present':'MISSING')+' gemini:'+(GK?'present':'MISSING'));
  await storage.ensureBucket();
  const SBP = process.env.SUPABASE_URL+'/storage/v1/object/public/'+storage.BUCKET;
  const man = { made_at:new Date().toISOString(), film:FILM.nm, brands:{ tutorialregister:{ nm:FILM.nm } } };
  const g = man.brands.tutorialregister;
  // seed from existing manifest so a partial re-run never blanks finished media
  try{ const prev=await fetch(SBP+'/tutorialregister/manifest.json?cb='+Date.now());
    if(prev.ok){ const pj=await prev.json(); if(pj&&pj.brands&&pj.brands.tutorialregister) Object.assign(g, pj.brands.tutorialregister); } }catch(e){}
  ...
  if(wantVoice){
    try{ g.voice = await tts(); console.log('TUTORIALREGISTER voice OK (new take — retime captions)'); }
    catch(e){ console.error('TUTORIALREGISTER VOICE FAIL',e.message); g.voice_error=String(e.message).slice(0,200); }
  } else { console.log('TUTORIALREGISTER voice REUSED'); }
  // music — reuse unless redo
  try{
    const mu = SBP+'/tutorialregister/tutorialregister-music.mp3';
    const mhead = redo ? {ok:false} : await fetch(mu,{method:'HEAD'});
    if(mhead.ok){ g.music = mu+'?v='+V; console.log('TUTORIALREGISTER music REUSED'); }
    else { g.music = await lyria(); console.log('TUTORIALREGISTER music OK'); }
  }catch(e){ console.error('TUTORIALREGISTER MUSIC FAIL',e.message); g.music_error=String(e.message).slice(0,200); }
  // scenes — reuse unless redo; checkpoint after each so a killed run never loses finished media
  g.shots = g.shots || [];
  for(let ix=1; ix<=STORY.length; ix++){
    try{
      const u = SBP+'/tutorialregister/tutorialregister-scene'+ix+'.mp4';
      const head = (redo || ONLY.indexOf(ix)>=0) ? {ok:false} : await fetch(u,{method:'HEAD'});
      if(head.ok){ g.shots[ix-1]=u+'?v='+V; console.log('TUTORIALREGISTER scene REUSED',ix); }
      else { g.shots[ix-1]=await veo(STORY[ix-1], ix); console.log('TUTORIALREGISTER scene OK',ix); }
    }catch(e){ console.error('TUTORIALREGISTER SCENE FAIL',ix,e.message);
    ...
    console.log('TUTORIALREGISTER checkpoint',ix);
  }
  await saveManifest(man);
  console.log('TUTORIALREGISTER COMPLETE RUN',RUN, JSON.stringify(man));
