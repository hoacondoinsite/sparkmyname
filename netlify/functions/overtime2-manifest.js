// OVERTIME2 manifest — serves the marquee film's media (scenes + voice + music) to the player.
// No keys exposed. Reads only the isolated overtime2/ folder. Reel Law: never reads any other reel.
const storage = require('./sb-storage.js');
const SB_URL = process.env.SUPABASE_URL;
exports.handler = async () => {
  try{
    const r = await fetch(SB_URL+'/storage/v1/object/public/'+storage.BUCKET+'/overtime2/manifest.json?cb='+Date.now(),{cache:'no-store'});
