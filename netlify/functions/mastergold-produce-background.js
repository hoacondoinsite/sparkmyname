// MASTERGOLD — standalone cinematic producer for the SparkMyName homepage film.
// MASTER GOLD v4 (Vinello Edition) — combined master: Premiere story spine + Gold Reel craft.
// 100% isolated: its OWN file, its OWN storage folder (mastergold/), its OWN manifest, its OWN single film.
...
const FILM = {
  ...
  voice: 'onyx',
  vibe: "Mid-age American voice, warm and grounded \u2014 a kind friend who finally shows you how, never a announcer. Gentle and sincere on 'Nobody ever showed you.' Encouraging and plain through the demo lines. Honest, unhurried clarity on 'still available' and 'ready to print.' Lands 'Say it... and see' soft, warm, and inviting \u2014 a door held open. Honor every ellipsis as a real pause. Clear, well-projected studio level, always above the music.",
  script: "You've always had the idea. You just never knew where to start. Nobody ever showed you. So just say it \u2014 out loud, in your own words. One sentence. Telling us costs nothing. A name. Your logo. Your colors. And we check \u2014 right then \u2014 that the website name is still available. Your cards, your shirts, your Facebook page \u2014 first posts written. Your letterhead. Every design ready to print, done for you. Print them anywhere. It's all saved in your own account \u2014 yours to keep, with a guide for every step. Your idea. Your name. Say it... and see."
};
...
const STORY = [
 ...
 "Cinematic brand film, ONE continuous move, no cuts, the camera never stops. The actual product screen, shown honestly as clean elegant UI on the phone and blooming outward: a business name 'BrightNest Cleaning Co.' types itself in; a simple friendly emblem \u2014 a little house with a soft shine mark \u2014 draws itself in deep teal; a small palette of brand colors snaps into place with hex chips; elegant type specimens; and beside the matching website name brightnestcleaning.com a bright green AVAILABLE check lands like a small victory. Real interface realism, not fantasy. BRAND LOCK \u2014 the name reads exactly 'BrightNest Cleaning Co.'. Strictly exclude: real logos, garbled text, cuts. Photorealistic premium UI motion design, deep teal, white, warm gray, a touch of gold from the Spark interface.",
...
async function tts(){
  ...
  const up = await storage.uploadPng('mastergold/mastergold-voice.mp3', b64, 'audio/mpeg');
...
async function lyria(){
  ...
  const up = await storage.uploadPng('mastergold/mastergold-music.mp3', aud.data, mime);
...
async function veo(prompt, ix){
  ...
  for(const model of MODELS){
    for(let a=1;a<=2;a++){
      ...
      if(!r.ok){
        ...
        console.error('MASTERGOLD veo start FAIL',ix,model,'try',a,r.status,t.slice(0,240));
        ...
        if(/personGeneration|person_generation/i.test(t)){
          ...
          if(r.ok){ op=await r.json(); console.log('MASTERGOLD veo OK (no-person)',model,ix); break outer; }
          lastErr='veo '+model+' person retry '+r.status; break; }
        lastErr='veo '+model+' '+r.status+' '+t; break;
      } else { op=await r.json(); console.log('MASTERGOLD veo OK',model,ix); break outer; }
    }
  }
  ...
  const up = await storage.uploadPng('mastergold/mastergold-scene'+ix+'.mp4', b64, 'video/mp4');
...
async function saveManifest(man){
  try{
    ...
    await storage.uploadPng('mastergold/manifest.json', mb64, 'application/json');
  }catch(e){ console.error('MASTERGOLD MANIFEST SAVE FAIL', e.message); }
}
exports.handler = async (event) => {
  ...
  console.log('MASTERGOLD BUILD RUN',RUN,'redo='+(q.redo||'(none)')+' only='+(q.only||'(none)'));
  console.log('MASTERGOLD KEYS openai:'+(OA?'present':'MISSING')+' gemini:'+(GK?'present':'MISSING'));
  await storage.ensureBucket();
  const SBP = process.env.SUPABASE_URL+'/storage/v1/object/public/'+storage.BUCKET;
  const man = { made_at:new Date().toISOString(), film:FILM.nm, brands:{ mastergold:{ nm:FILM.nm } } };
  const g = man.brands.mastergold;
  // seed from existing manifest so a partial re-run never blanks finished media
  try{ const prev=await fetch(SBP+'/mastergold/manifest.json?cb='+Date.now());
    if(prev.ok){ const pj=await prev.json(); if(pj&&pj.brands&&pj.brands.mastergold) Object.assign(g, pj.brands.mastergold); } }catch(e){}
  ...
  try{ g.voice = await tts(); console.log('MASTERGOLD voice OK'); }
  catch(e){ console.error('MASTERGOLD VOICE FAIL',e.message); g.voice_error=String(e.message).slice(0,200); }
  // music — reuse unless redo
  try{
    const mu = SBP+'/mastergold/mastergold-music.mp3';
    const mhead = redo ? {ok:false} : await fetch(mu,{method:'HEAD'});
    if(mhead.ok){ g.music = mu+'?v='+V; console.log('MASTERGOLD music REUSED'); }
    else { g.music = await lyria(); console.log('MASTERGOLD music OK'); }
  }catch(e){ console.error('MASTERGOLD MUSIC FAIL',e.message); g.music_error=String(e.message).slice(0,200); }
  // scenes — reuse unless redo; checkpoint after each so a killed run never loses finished media
  g.shots = g.shots || [];
  for(let ix=1; ix<=STORY.length; ix++){
    try{
      const u = SBP+'/mastergold/mastergold-scene'+ix+'.mp4';
      const head = (redo || ONLY.indexOf(ix)>=0) ? {ok:false} : await fetch(u,{method:'HEAD'});
      if(head.ok){ g.shots[ix-1]=u+'?v='+V; console.log('MASTERGOLD scene REUSED',ix); }
      else { g.shots[ix-1]=await veo(STORY[ix-1], ix); console.log('MASTERGOLD scene OK',ix); }
    }catch(e){ console.error('MASTERGOLD SCENE FAIL',ix,e.message);
    ...
    console.log('MASTERGOLD checkpoint',ix);
  }
  await saveManifest(man);
  console.log('MASTERGOLD COMPLETE RUN',RUN, JSON.stringify(man));
