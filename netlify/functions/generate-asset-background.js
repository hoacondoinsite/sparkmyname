...
async function notify(email, brandName, label, url) {
  if (!RESEND || !email) return { sent: false, reason: 'no_mail_config' };
  const html =
    '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0;background:#F0F7F3;"><div style="font-family:Inter,Arial,sans-serif;background:#FFFFFF;color:#EAF2FF;padding:32px">' +
    '<div style="max-width:520px;margin:0 auto;background:#F7FBF9;border-radius:12px;padding:28px">' +
    '<div style="font-weight:800;font-size:20px;margin-bottom:6px">Your ' + esc(label) + ' is ready</div>' +
    '<div style="color:#AFC2E1;font-size:15px;line-height:1.6;margin-bottom:22px">' +
    'We finished the ' + esc(label) + ' for <b>' + esc(brandName) + '</b>. It is waiting in your workspace.</div>' +
    '<a href="' + esc(url) + '" style="display:inline-block;background:#189850;color:#FFFFFF;' +
    'text-decoration:none;font-weight:700;padding:13px 26px;border-radius:12px">Download it</a>' +
    '</div></div>';
  try {
    const r = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + RESEND, 'Content-Type': 'application/json' },
      body: JSON.stringify({ from: FROM, to: [email], subject: 'Your ' + label + ' is ready', text: plainTextFrom(html, url), html: html })
    });
    return { sent: r.status < 300, status: r.status };
...
const SYNC_JUDGE_BUDGET_MS = parseInt(process.env.SMN_BG_JUDGE_BUDGET_MS || '600000', 10);

// BACKGROUND TWIN of generate-asset.js (2026-07-27).
//
// Netlify kills a synchronous function at 26 seconds. A 2K render takes 21-25 of them, so the
// dispatcher was dying with nothing saved — no asset, no charge, an empty shelf and the word
// "failed". The runs that succeeded earlier were luck, not headroom.
//
// A function whose name ends in -background gets a 15-minute ceiling. All the slow work lives
// here: spec resolution, the concept desk, the render, the judge and its retries, the upload
// and the kit write. The dispatcher does only the fast gates and hands off.
//
// The two files are deliberate twins rather than one shared module: the delivery path is not
// the place for a refactor at the moment it is finally working. If the pipeline changes,
// BOTH must change. This comment is the reminder.
...
exports.handler = async function (event) {
  ...
  // EXCEPTION (2026-07-30, Founder order): mirror of generate-asset.js — a PRESENTATION STUDY is
  // allowed for any OWNED name, activated or not, so every one of the six names can be shown in
  // the world. Same safety: an openMode wantsMockup request can ONLY build a study; no other paid
  // asset slips through, and ownership above is still required. Keep in lockstep with the twin.
  if (kit._activated !== true && !(openMode && wantsMockup(spec.request_text))) return fail(402, 'brand_not_activated');
  ...
  // THE CLAIM LIVES HERE (2026-07-27). Taken on this side rather than by the dispatcher, so a
  // dispatch that never arrives leaves no claim behind and the customer can retry the same
  // words immediately. Two rapid clicks both dispatch; the first claim wins, the second exits.
  try {
    const claim = await storage.claimOnce('idem/' + idemHash + '.claim', new Date().toISOString());
    if (claim && claim.claimed === false) return fail(409, 'duplicate_request_in_progress');
  } catch (e) { /* storage unreachable — fail open rather than block a paid customer */ }

  // GATE 5 — spend cap, reserved before the call.
  const spendKey = storeKey;
  if (!spendReserve(kit, spendKey)) {
    return fail(429, 'spend_cap_reached', { spent: spendTotal(kit), cap: r4(CAP_USD) });
  }

  // ---- generate: exactly one asset ---------------------------------------------------------
  const wo = openMode
    ? (wantsMockup(spec.request_text)
        ? await mockupWorkOrder(spec, name, kit)
        : wantsIdentity(spec.request_text)
          ? await identityWorkOrder(spec, name, kit, wantsSmallMark(spec.request_text))
          : openWorkOrder(spec, name, kit, answers))
    : translator.workOrder({
        format: assetType, name: name, domain: kit.domain || '', seed: kit.seed || '',
        kit: kit, brief: kit.gov || null, markUrl: (kit.logoUrls && kit.logoUrls[0]) || null
      });
  if (!wo || wo.error) {
    spendRelease(kit, spendKey);
    await saveKit(reportId, dbRow.position, kit);
    return fail(500, 'work_order_failed', { detail: (wo && wo.error) || 'no work order' });
  }

  const path = 'assets/' + reportId + '/' + safeSeg(name, 60) + '/' + storeKey + '.png';
  let url = null, charged = 0;

  if (wo.engine === 'svg' && wo.svg) {
    // Vector rows cost nothing — no API call, no charge.
    spendRelease(kit, spendKey);
    try {
      await storage.ensureBucket();
      const p = 'assets/' + reportId + '/' + safeSeg(name, 60) + '/' + storeKey + '.svg';
      const up = await storage.uploadPng(p, Buffer.from(wo.svg, 'utf-8').toString('base64'), 'image/svg+xml');
      if (up && up.ok !== false && up.url) url = up.url;
    } catch (e) { url = null; }
    if (!url) return fail(500, 'svg_storage_failed');
  } else {
    const basePrompt = (wo.prompts && wo.prompts[0]) || '';
    const engOpts = {
      /* uses the house PHOTO_LADDER in studio-engine (2026-07-27) — no ad-hoc tier lists */ imageSize: (wo.spec && wo.spec.size) || '2K',
      aspectRatio: (wo.spec && wo.spec.aspect) || '16:9'
    };

    let img = null, verdict = null;

    // THE JUDGE RUNS ON IDENTITY WORK ONLY (2026-07-27, Founder order). A photograph is judged
    // by the person who asked for it; a logo is judged against a written law, and that law can
    // be checked by something that can see. Rendered mark -> vision judge -> if it fails, render
    // again TOLD WHAT WAS WRONG. Best attempt ships. See docs/SPARK_LOGO_LAW.md.
    //
    // Photographs and presentation studies keep the single-shot path: no judge, no retries, no
    // change in cost or behaviour from what was tested tonight.
    if (openMode && wantsIdentity(spec.request_text) && !wantsMockup(spec.request_text)) {
      // Judge only if the clock allows. Past the budget, render once and ship — an unjudged
      // mark on the shelf beats a killed invocation and an empty screen.
      const roomToJudge = (Date.now() - T0) < SYNC_JUDGE_BUDGET_MS;
      const res = await judged.renderBestOf({
        name: name,
        attempts: 1,               // single pass (2026-07-27, Founder order)
        budgetMs: 600000,
        render: async function (correction) {
          const prompt = correction ? (basePrompt + ' ' + correction) : basePrompt;
          try { return await engine.generateImage(prompt, engOpts); } catch (e) { return null; }
        },
        judge: roomToJudge ? undefined : async function () {
          return { pass: true, unknown: true, reason: 'no_time_on_sync_path',
                   total: null, verdict: 'unknown' };
        }
      });
      if (res) { img = res.img; verdict = res.verdict; verdict.attempts_used = res.attempts_used; }
    } else {
      try { img = await engine.generateImage(basePrompt, engOpts); } catch (e) { img = null; }
    }

    if (!img || !img.ok || !img.b64) {
      spendRelease(kit, spendKey);
      await saveKit(reportId, dbRow.position, kit);
      return fail(502, 'generation_failed');
    }
    charged = r4(typeof img.costEst === 'number' ? img.costEst : FALLBACK_COST);
    spendSettle(kit, spendKey, charged);
    ...
    if (!url) return fail(500, 'storage_failed', { note: 'generated but not stored; spend recorded' });
  }

  // ---- persist + notify --------------------------------------------------------------------
  kit.assets[storeKey] = {
    url: url, label: row.label || assetType, engine: wo.engine,
    cost: charged, at: new Date().toISOString()
  };
  if (verdict) {
    kit.assets[storeKey].judged = {
      total: verdict.total, verdict: verdict.verdict,
      attempts: verdict.attempts_used || 1,
      failed_gates: verdict.failed_gates || [],
      codex: verdict.codex || {},
      failed_axioms: verdict.failed_axioms || [],
      one_line: verdict.one_line || ''
    };
  }
  if (openMode) {
    kit.assets[storeKey].open = true;
    kit.assets[storeKey].request = spec.request_text;
    kit.assets[storeKey].spec = {
      width: spec.width, height: spec.height, dpi: spec.dpi,
      bleed: spec.bleed, source: spec.source
    };
  }
  await saveKit(reportId, dbRow.position, kit);

  const mail = await notify(email, name, row.label || assetType, SITE + '/workspace.html?r=' + encodeURIComponent(reportId));
  ...
  return resp(200, {
    ...
    open: openMode,
    spec: openMode ? { width: spec.width, height: spec.height, dpi: spec.dpi,
                       bleed: spec.bleed, source: spec.source } : null,
    concept: (wo && wo.concept) ? { idea: wo.concept.idea, secondary: wo.concept.secondary,
                                    survives: wo.concept.survives } : null,
    judged: verdict ? { total: verdict.total, verdict: verdict.verdict,
                        attempts: verdict.attempts_used || 1,
                        codex: verdict.codex || {},
                        failed_axioms: verdict.failed_axioms || [],
                        one_line: verdict.one_line || '' } : null,
    cost: charged,
    spent: spendTotal(kit),
    cap: r4(CAP_USD),
    notified: !!(mail && mail.sent)
  });
};
