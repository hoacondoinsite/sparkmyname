// TUTORIALTRADEMARK — standalone tutorial-video producer for the SparkMyName Live Agent guide.
...
// gets a BRAND-NEW name (producer + storage folder + manifest): tutorialtrademark is virgin, permanent, never reused.
// Old reels are frozen history — never read, never written.
// ============================================================
// Check & Protect Your Name — a single warm narrator (no customer cameos) + Lyria music + 5 Veo scenes,
// a calm instructional "watch how" tutorial cut for one Live Agent step.
// 100% isolated: its OWN file, its OWN storage folder (tutorialtrademark/), its OWN manifest, its OWN single film.
...
const FILM = {
  slug: "tutorialtrademark",   // virgin, permanent, never reused
  nm: "Check & Protect Your Name",
  ...
  script: "Now let's protect your name. Go to the government's free trademark website. Search your business name, and see if anyone else uses it. If it's open, you can start to register it as yours. This helps you check and register — it is not legal advice. Take your time; there's no rush. You've got this."
};
...
// STORY — 5 Veo prompts. Each core has the standing ARMOR APPENDED VERBATIM (inlined per take).
const STORY = [
 // T1
 "A person leans toward a laptop in a calm home office, unhurried. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T2
 "Hands type a search on a laptop; a soft magnifying-glass motion sweeps across the screen; text-free. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T3
 "A simple shield emblem draws itself in deep gold — the protected idea, shapes only. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T4
 "The person nods slowly, reassured — no worry on their face. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T5
 "A soft settle on their calm hands folded beside the laptop. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts."
];
...
async function tts(){
  ...
  const up = await storage.uploadPng('tutorialtrademark/tutorialtrademark-voice.mp3', b64, 'audio/mpeg');
...
async function lyria(){
  ...
  const up = await storage.uploadPng('tutorialtrademark/tutorialtrademark-music.mp3', aud.data, mime);
...
async function veo(prompt, ix){
  ...
  for(const model of MODELS){
    for(let a=1;a<=2;a++){
      ...
      if(!r.ok){
        ...
        console.error('TUTORIALTRADEMARK veo start FAIL',ix,model,'try',a,r.status,t.slice(0,240));
        ...
        if(/personGeneration|person_generation/i.test(t)){
          ...
          if(r.ok){ op=await r.json(); console.log('TUTORIALTRADEMARK veo OK (no-person)',model,ix); break outer; }
          lastErr='veo '+model+' person retry '+r.status; break; }
        lastErr='veo '+model+' '+r.status+' '+t; break;
      } else { op=await r.json(); console.log('TUTORIALTRADEMARK veo OK',model,ix); break outer; }
    }
  }
  ...
  const up = await storage.uploadPng('tutorialtrademark/tutorialtrademark-scene'+ix+'.mp4', b64, 'video/mp4');
...
async function saveManifest(man){
  try{
    ...
    await storage.uploadPng('tutorialtrademark/manifest.json', mb64, 'application/json');
  }catch(e){ console.error('TUTORIALTRADEMARK MANIFEST SAVE FAIL', e.message); }
}
exports.handler = async (event) => {
  ...
  console.log('TUTORIALTRADEMARK BUILD RUN',RUN,'redo='+(q.redo||'(none)')+' only='+(q.only||'(none)'));
  console.log('TUTORIALTRADEMARK KEYS openai:'+(OA?'present':'MISSING')+' gemini:'+(GK?'present':'MISSING'));
  await storage.ensureBucket();
  const SBP = process.env.SUPABASE_URL+'/storage/v1/object/public/'+storage.BUCKET;
  const man = { made_at:new Date().toISOString(), film:FILM.nm, brands:{ tutorialtrademark:{ nm:FILM.nm } } };
  const g = man.brands.tutorialtrademark;
  // seed from existing manifest so a partial re-run never blanks finished media
  try{ const prev=await fetch(SBP+'/tutorialtrademark/manifest.json?cb='+Date.now());
    if(prev.ok){ const pj=await prev.json(); if(pj&&pj.brands&&pj.brands.tutorialtrademark) Object.assign(g, pj.brands.tutorialtrademark); } }catch(e){}
  ...
  if(wantVoice){
    try{ g.voice = await tts(); console.log('TUTORIALTRADEMARK voice OK (new take — retime captions)'); }
    catch(e){ console.error('TUTORIALTRADEMARK VOICE FAIL',e.message); g.voice_error=String(e.message).slice(0,200); }
  } else { console.log('TUTORIALTRADEMARK voice REUSED'); }
  // music — reuse unless redo
  try{
    const mu = SBP+'/tutorialtrademark/tutorialtrademark-music.mp3';
    const mhead = redo ? {ok:false} : await fetch(mu,{method:'HEAD'});
    if(mhead.ok){ g.music = mu+'?v='+V; console.log('TUTORIALTRADEMARK music REUSED'); }
    else { g.music = await lyria(); console.log('TUTORIALTRADEMARK music OK'); }
  }catch(e){ console.error('TUTORIALTRADEMARK MUSIC FAIL',e.message); g.music_error=String(e.message).slice(0,200); }
  // scenes — reuse unless redo; checkpoint after each so a killed run never loses finished media
  g.shots = g.shots || [];
  for(let ix=1; ix<=STORY.length; ix++){
    try{
      const u = SBP+'/tutorialtrademark/tutorialtrademark-scene'+ix+'.mp4';
      const head = (redo || ONLY.indexOf(ix)>=0) ? {ok:false} : await fetch(u,{method:'HEAD'});
      if(head.ok){ g.shots[ix-1]=u+'?v='+V; console.log('TUTORIALTRADEMARK scene REUSED',ix); }
      else { g.shots[ix-1]=await veo(STORY[ix-1], ix); console.log('TUTORIALTRADEMARK scene OK',ix); }
    }catch(e){ console.error('TUTORIALTRADEMARK SCENE FAIL',ix,e.message);
    ...
    console.log('TUTORIALTRADEMARK checkpoint',ix);
  }
  await saveManifest(man);
  console.log('TUTORIALTRADEMARK COMPLETE RUN',RUN, JSON.stringify(man));
