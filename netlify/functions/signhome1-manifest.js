// SIGNHOME1 manifest — serves the homepage film's media (scenes + voice + music) to the player.
// No keys exposed. Reads only the isolated signhome1/ folder. Reel Law: never reads any other reel.
const storage = require('./sb-storage.js');
const SB_URL = process.env.SUPABASE_URL;
exports.handler = async () => {
  try{
    const r = await fetch(SB_URL+'/storage/v1/object/public/'+storage.BUCKET+'/signhome1/manifest.json?cb='+Date.now(),{cache:'no-store'});
