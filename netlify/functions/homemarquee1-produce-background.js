...
// gets a BRAND-NEW name (producer + storage folder + manifest): homemarquee1 is virgin, permanent, never reused.
...
// 100% isolated: its OWN file, its OWN storage folder (homemarquee1/), its OWN manifest, its OWN single film.
...
const FILM = {
  slug: 'homemarquee1',   // virgin, permanent, never reused
  nm: 'TRUEHOME',
...
// master track at their exact timecodes, then the master uploads as homemarquee1-voice.mp3 and is voice-locked.
...
// name homemarquee1/homemarquee1-voice.mp3 (the manifest/player contract requires that exact name; the site player builds
...
async function tts(){
  ...
  const up=await storage.uploadPng('homemarquee1/homemarquee1-voice.mp3', b64, 'audio/wav');
...
async function lyria(){
  ...
  const up = await storage.uploadPng('homemarquee1/homemarquee1-music.mp3', aud.data, mime);
...
async function veo(prompt, ix){
  ...
  const up = await storage.uploadPng('homemarquee1/homemarquee1-scene'+ix+'.mp4', b64, 'video/mp4');
...
async function saveManifest(man){
  try{
    ...
    await storage.uploadPng('homemarquee1/manifest.json', mb64, 'application/json');
...
exports.handler = async (event) => {
  ...
  const man = { made_at:new Date().toISOString(), film:FILM.nm, brands:{ homemarquee1:{ nm:FILM.nm } } };
  const g = man.brands.homemarquee1;
  // seed from existing manifest so a partial re-run never blanks finished media
  try{ const prev=await fetch(SBP+'/homemarquee1/manifest.json?cb='+Date.now());
    if(prev.ok){ const pj=await prev.json(); if(pj&&pj.brands&&pj.brands.homemarquee1) Object.assign(g, pj.brands.homemarquee1); } }catch(e){}
  ...
  try{
    const mu = SBP+'/homemarquee1/homemarquee1-music.mp3';
  ...
  for(let ix=1; ix<=STORY.length; ix++){
    try{
      const u = SBP+'/homemarquee1/homemarquee1-scene'+ix+'.mp4';
