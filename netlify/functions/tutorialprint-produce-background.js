// TUTORIALPRINT — standalone tutorial-video producer for the SparkMyName Live Agent guide.
...
// gets a BRAND-NEW name (producer + storage folder + manifest): tutorialprint is virgin, permanent, never reused.
// Old reels are frozen history — never read, never written.
// ============================================================
// Print Your Things — a single warm narrator (no customer cameos) + Lyria music + 4 Veo scenes,
// a calm instructional "watch how" tutorial cut for one Live Agent step.
// 100% isolated: its OWN file, its OWN storage folder (tutorialprint/), its OWN manifest, its OWN single film.
...
const FILM = {
  slug: "tutorialprint",   // virgin, permanent, never reused
  nm: "Print Your Things",
  ...
  script: "Time to print your things — your cards, flyers, and signs are ready in your workspace. Print them at our store, or anywhere you like. It's your choice. Hold your brand in your hands. You've got this."
};
...
const STORY = [
 // T1
 "A phone in a hand showing a warm workspace of finished designs as clean abstract tiles, thumb scrolling; text-free. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T2
 "Clean business cards sliding out of a printer, crisp — abstract clean cards, no readable text. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T3
 "Hands pick up a fresh business card and turn it over with quiet pride. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T4
 "A tidy stack of cards and a flyer on a table in warm light — real, in hand. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts."
];
...
async function tts(){
  ...
  const up = await storage.uploadPng('tutorialprint/tutorialprint-voice.mp3', b64, 'audio/mpeg');
...
async function lyria(){
  ...
  const up = await storage.uploadPng('tutorialprint/tutorialprint-music.mp3', aud.data, mime);
...
async function veo(prompt, ix){
  ...
  for(const model of MODELS){
    for(let a=1;a<=2;a++){
      ...
      if(!r.ok){
        ...
        console.error('TUTORIALPRINT veo start FAIL',ix,model,'try',a,r.status,t.slice(0,240));
        ...
        if(/personGeneration|person_generation/i.test(t)){
          ...
          if(r.ok){ op=await r.json(); console.log('TUTORIALPRINT veo OK (no-person)',model,ix); break outer; }
          lastErr='veo '+model+' person retry '+r.status; break; }
        lastErr='veo '+model+' '+r.status+' '+t; break;
      } else { op=await r.json(); console.log('TUTORIALPRINT veo OK',model,ix); break outer; }
    }
  }
  ...
  const up = await storage.uploadPng('tutorialprint/tutorialprint-scene'+ix+'.mp4', b64, 'video/mp4');
...
async function saveManifest(man){
  try{
    ...
    await storage.uploadPng('tutorialprint/manifest.json', mb64, 'application/json');
  }catch(e){ console.error('TUTORIALPRINT MANIFEST SAVE FAIL', e.message); }
}
exports.handler = async (event) => {
  ...
  console.log('TUTORIALPRINT BUILD RUN',RUN,'redo='+(q.redo||'(none)')+' only='+(q.only||'(none)'));
  console.log('TUTORIALPRINT KEYS openai:'+(OA?'present':'MISSING')+' gemini:'+(GK?'present':'MISSING'));
  await storage.ensureBucket();
  const SBP = process.env.SUPABASE_URL+'/storage/v1/object/public/'+storage.BUCKET;
  const man = { made_at:new Date().toISOString(), film:FILM.nm, brands:{ tutorialprint:{ nm:FILM.nm } } };
  const g = man.brands.tutorialprint;
  // seed from existing manifest so a partial re-run never blanks finished media
  try{ const prev=await fetch(SBP+'/tutorialprint/manifest.json?cb='+Date.now());
    if(prev.ok){ const pj=await prev.json(); if(pj&&pj.brands&&pj.brands.tutorialprint) Object.assign(g, pj.brands.tutorialprint); } }catch(e){}
  ...
  if(wantVoice){
    try{ g.voice = await tts(); console.log('TUTORIALPRINT voice OK (new take — retime captions)'); }
    catch(e){ console.error('TUTORIALPRINT VOICE FAIL',e.message); g.voice_error=String(e.message).slice(0,200); }
  } else { console.log('TUTORIALPRINT voice REUSED'); }
  // music — reuse unless redo
  try{
    const mu = SBP+'/tutorialprint/tutorialprint-music.mp3';
    const mhead = redo ? {ok:false} : await fetch(mu,{method:'HEAD'});
    if(mhead.ok){ g.music = mu+'?v='+V; console.log('TUTORIALPRINT music REUSED'); }
    else { g.music = await lyria(); console.log('TUTORIALPRINT music OK'); }
  }catch(e){ console.error('TUTORIALPRINT MUSIC FAIL',e.message); g.music_error=String(e.message).slice(0,200); }
  // scenes — reuse unless redo; checkpoint after each so a killed run never loses finished media
  g.shots = g.shots || [];
  for(let ix=1; ix<=STORY.length; ix++){
    try{
      const u = SBP+'/tutorialprint/tutorialprint-scene'+ix+'.mp4';
      const head = (redo || ONLY.indexOf(ix)>=0) ? {ok:false} : await fetch(u,{method:'HEAD'});
      if(head.ok){ g.shots[ix-1]=u+'?v='+V; console.log('TUTORIALPRINT scene REUSED',ix); }
      else { g.shots[ix-1]=await veo(STORY[ix-1], ix); console.log('TUTORIALPRINT scene OK',ix); }
    }catch(e){ console.error('TUTORIALPRINT SCENE FAIL',ix,e.message);
    ...
    console.log('TUTORIALPRINT checkpoint',ix);
  }
  await saveManifest(man);
  console.log('TUTORIALPRINT COMPLETE RUN',RUN, JSON.stringify(man));
