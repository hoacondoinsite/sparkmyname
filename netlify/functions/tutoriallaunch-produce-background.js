// TUTORIALLAUNCH — standalone tutorial-video producer for the SparkMyName Live Agent guide.
...
// gets a BRAND-NEW name (producer + storage folder + manifest): tutoriallaunch is virgin, permanent, never reused.
// Old reels are frozen history — never read, never written.
// ============================================================
// Launch Your Pages — a single warm narrator (no customer cameos) + Lyria music + 4 Veo scenes,
// a calm instructional "watch how" tutorial cut for one Live Agent step.
// 100% isolated: its OWN file, its OWN storage folder (tutoriallaunch/), its OWN manifest, its OWN single film.
...
const FILM = {
  slug: "tutoriallaunch",   // virgin, permanent, never reused
  nm: "Launch Your Pages",
  ...
  script: "Last step — launch your pages. Your first posts are already written and waiting in your workspace. Open your social page, and put them up. Now the world can find you. You're open for business. You've got this."
};
...
const STORY = [
 // T1
 "A person on a couch with a phone, relaxed, a small excited smile. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T2
 "Thumb taps to set up a social page as text-free UI shapes; a profile circle fills with warm color. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T3
 "A post card rises into place; a small heart animates beside it — shapes only, no words. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T4
 "The person looks up from the phone, quietly thrilled — they're live. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts."
];
...
async function tts(){
  ...
  const up = await storage.uploadPng('tutoriallaunch/tutoriallaunch-voice.mp3', b64, 'audio/mpeg');
...
async function lyria(){
  ...
  const up = await storage.uploadPng('tutoriallaunch/tutoriallaunch-music.mp3', aud.data, mime);
...
async function veo(prompt, ix){
  ...
  for(const model of MODELS){
    for(let a=1;a<=2;a++){
      ...
      if(!r.ok){
        ...
        console.error('TUTORIALLAUNCH veo start FAIL',ix,model,'try',a,r.status,t.slice(0,240));
        ...
        if(/personGeneration|person_generation/i.test(t)){
          ...
          if(r.ok){ op=await r.json(); console.log('TUTORIALLAUNCH veo OK (no-person)',model,ix); break outer; }
          lastErr='veo '+model+' person retry '+r.status; break; }
        lastErr='veo '+model+' '+r.status+' '+t; break;
      } else { op=await r.json(); console.log('TUTORIALLAUNCH veo OK',model,ix); break outer; }
    }
  }
  ...
  const up = await storage.uploadPng('tutoriallaunch/tutoriallaunch-scene'+ix+'.mp4', b64, 'video/mp4');
...
async function saveManifest(man){
  try{
    ...
    await storage.uploadPng('tutoriallaunch/manifest.json', mb64, 'application/json');
  }catch(e){ console.error('TUTORIALLAUNCH MANIFEST SAVE FAIL', e.message); }
}
exports.handler = async (event) => {
  ...
  console.log('TUTORIALLAUNCH BUILD RUN',RUN,'redo='+(q.redo||'(none)')+' only='+(q.only||'(none)'));
  console.log('TUTORIALLAUNCH KEYS openai:'+(OA?'present':'MISSING')+' gemini:'+(GK?'present':'MISSING'));
  await storage.ensureBucket();
  const SBP = process.env.SUPABASE_URL+'/storage/v1/object/public/'+storage.BUCKET;
  const man = { made_at:new Date().toISOString(), film:FILM.nm, brands:{ tutoriallaunch:{ nm:FILM.nm } } };
  const g = man.brands.tutoriallaunch;
  // seed from existing manifest so a partial re-run never blanks finished media
  try{ const prev=await fetch(SBP+'/tutoriallaunch/manifest.json?cb='+Date.now());
    if(prev.ok){ const pj=await prev.json(); if(pj&&pj.brands&&pj.brands.tutoriallaunch) Object.assign(g, pj.brands.tutoriallaunch); } }catch(e){}
  ...
  if(wantVoice){
    try{ g.voice = await tts(); console.log('TUTORIALLAUNCH voice OK (new take — retime captions)'); }
    catch(e){ console.error('TUTORIALLAUNCH VOICE FAIL',e.message); g.voice_error=String(e.message).slice(0,200); }
  } else { console.log('TUTORIALLAUNCH voice REUSED'); }
  // music — reuse unless redo
  try{
    const mu = SBP+'/tutoriallaunch/tutoriallaunch-music.mp3';
    const mhead = redo ? {ok:false} : await fetch(mu,{method:'HEAD'});
    if(mhead.ok){ g.music = mu+'?v='+V; console.log('TUTORIALLAUNCH music REUSED'); }
    else { g.music = await lyria(); console.log('TUTORIALLAUNCH music OK'); }
  }catch(e){ console.error('TUTORIALLAUNCH MUSIC FAIL',e.message); g.music_error=String(e.message).slice(0,200); }
  // scenes — reuse unless redo; checkpoint after each so a killed run never loses finished media
  g.shots = g.shots || [];
  for(let ix=1; ix<=STORY.length; ix++){
    try{
      const u = SBP+'/tutoriallaunch/tutoriallaunch-scene'+ix+'.mp4';
      const head = (redo || ONLY.indexOf(ix)>=0) ? {ok:false} : await fetch(u,{method:'HEAD'});
      if(head.ok){ g.shots[ix-1]=u+'?v='+V; console.log('TUTORIALLAUNCH scene REUSED',ix); }
      else { g.shots[ix-1]=await veo(STORY[ix-1], ix); console.log('TUTORIALLAUNCH scene OK',ix); }
    }catch(e){ console.error('TUTORIALLAUNCH SCENE FAIL',ix,e.message);
    ...
    console.log('TUTORIALLAUNCH checkpoint',ix);
  }
  await saveManifest(man);
  console.log('TUTORIALLAUNCH COMPLETE RUN',RUN, JSON.stringify(man));
