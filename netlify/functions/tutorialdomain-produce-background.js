// TUTORIALDOMAIN — standalone tutorial-video producer for the SparkMyName Live Agent guide.
...
// gets a BRAND-NEW name (producer + storage folder + manifest): tutorialdomain is virgin, permanent, never reused.
// Old reels are frozen history — never read, never written.
// ============================================================
// Claim Your Web Address — a single warm narrator (no customer cameos) + Lyria music + 5 Veo scenes,
// a calm instructional "watch how" tutorial cut for one Live Agent step.
// 100% isolated: its OWN file, its OWN storage folder (tutorialdomain/), its OWN manifest, its OWN single film.
...
const FILM = {
  slug: "tutorialdomain",   // virgin, permanent, never reused
  nm: "Claim Your Web Address",
  ...
  script: "Let's claim your web address — it's the first thing to do. Open a web-address seller, like the ones your guide names. Type your name into the search box. If it's open, follow the steps to make it yours. Once it's yours, no one else can take it. That's it — your address is claimed. You've got this."
};
...
const STORY = [
 // T1
 "A person at a kitchen table opens a laptop in warm morning light, calm and ready. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T2
 "Close on hands typing into a plain search field on a laptop screen; a soft cursor blinks; no readable text. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T3
 "A large green checkmark draws itself beside the field with a gentle glow — the moment it's open; shapes only. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T4
 "The person's face lifts into a quiet, relieved smile; a hand rests on the trackpad. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts.",
 // T5
 "Pull back: the laptop closed, a warm cup beside it — done, at ease. ONE continuous shot, no cuts, the camera never stops. Warm realistic lighting, photorealistic and human. ABSOLUTE TEXT BAN — the video model renders no words or letters of any kind; every word the viewer reads is drawn by the site player as an overlay. Strictly exclude: real logos, readable text, garbled text, cuts."
];
...
async function tts(){
  ...
  const up = await storage.uploadPng('tutorialdomain/tutorialdomain-voice.mp3', b64, 'audio/mpeg');
...
async function lyria(){
  ...
  const up = await storage.uploadPng('tutorialdomain/tutorialdomain-music.mp3', aud.data, mime);
...
async function veo(prompt, ix){
  ...
  for(const model of MODELS){
    for(let a=1;a<=2;a++){
      ...
      if(!r.ok){
        ...
        console.error('TUTORIALDOMAIN veo start FAIL',ix,model,'try',a,r.status,t.slice(0,240));
        ...
        if(/personGeneration|person_generation/i.test(t)){
          ...
          if(r.ok){ op=await r.json(); console.log('TUTORIALDOMAIN veo OK (no-person)',model,ix); break outer; }
          lastErr='veo '+model+' person retry '+r.status; break; }
        lastErr='veo '+model+' '+r.status+' '+t; break;
      } else { op=await r.json(); console.log('TUTORIALDOMAIN veo OK',model,ix); break outer; }
    }
  }
  ...
  const up = await storage.uploadPng('tutorialdomain/tutorialdomain-scene'+ix+'.mp4', b64, 'video/mp4');
...
async function saveManifest(man){
  try{
    ...
    await storage.uploadPng('tutorialdomain/manifest.json', mb64, 'application/json');
  }catch(e){ console.error('TUTORIALDOMAIN MANIFEST SAVE FAIL', e.message); }
}
exports.handler = async (event) => {
  ...
  console.log('TUTORIALDOMAIN BUILD RUN',RUN,'redo='+(q.redo||'(none)')+' only='+(q.only||'(none)'));
  console.log('TUTORIALDOMAIN KEYS openai:'+(OA?'present':'MISSING')+' gemini:'+(GK?'present':'MISSING'));
  await storage.ensureBucket();
  const SBP = process.env.SUPABASE_URL+'/storage/v1/object/public/'+storage.BUCKET;
  const man = { made_at:new Date().toISOString(), film:FILM.nm, brands:{ tutorialdomain:{ nm:FILM.nm } } };
  const g = man.brands.tutorialdomain;
  // seed from existing manifest so a partial re-run never blanks finished media
  try{ const prev=await fetch(SBP+'/tutorialdomain/manifest.json?cb='+Date.now());
    if(prev.ok){ const pj=await prev.json(); if(pj&&pj.brands&&pj.brands.tutorialdomain) Object.assign(g, pj.brands.tutorialdomain); } }catch(e){}
  ...
  if(wantVoice){
    try{ g.voice = await tts(); console.log('TUTORIALDOMAIN voice OK (new take — retime captions)'); }
    catch(e){ console.error('TUTORIALDOMAIN VOICE FAIL',e.message); g.voice_error=String(e.message).slice(0,200); }
  } else { console.log('TUTORIALDOMAIN voice REUSED'); }
  // music — reuse unless redo
  try{
    const mu = SBP+'/tutorialdomain/tutorialdomain-music.mp3';
    const mhead = redo ? {ok:false} : await fetch(mu,{method:'HEAD'});
    if(mhead.ok){ g.music = mu+'?v='+V; console.log('TUTORIALDOMAIN music REUSED'); }
    else { g.music = await lyria(); console.log('TUTORIALDOMAIN music OK'); }
  }catch(e){ console.error('TUTORIALDOMAIN MUSIC FAIL',e.message); g.music_error=String(e.message).slice(0,200); }
  // scenes — reuse unless redo; checkpoint after each so a killed run never loses finished media
  g.shots = g.shots || [];
  for(let ix=1; ix<=STORY.length; ix++){
    try{
      const u = SBP+'/tutorialdomain/tutorialdomain-scene'+ix+'.mp4';
      const head = (redo || ONLY.indexOf(ix)>=0) ? {ok:false} : await fetch(u,{method:'HEAD'});
      if(head.ok){ g.shots[ix-1]=u+'?v='+V; console.log('TUTORIALDOMAIN scene REUSED',ix); }
      else { g.shots[ix-1]=await veo(STORY[ix-1], ix); console.log('TUTORIALDOMAIN scene OK',ix); }
    }catch(e){ console.error('TUTORIALDOMAIN SCENE FAIL',ix,e.message);
    ...
    console.log('TUTORIALDOMAIN checkpoint',ix);
  }
  await saveManifest(man);
  console.log('TUTORIALDOMAIN COMPLETE RUN',RUN, JSON.stringify(man));
